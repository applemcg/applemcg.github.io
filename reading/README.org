...
The frequently updated On My Nightstand reports
...
latestbook ()  { zcat .hry/book_page.rdb.Z | i_timeDate latest | jointable -1 book -2 book - author_book.rdb  | column latest  page author book | tee $(myname).rdb ; }

onMyNightstand () { latestbook | sorttable | link_authorBook; }; qf onMyNightstand


foreach rdb_canon {author_book,book_page}.rdb

onMyNightstand

: notquite readn for a function

zcat .hry/book_page.rdb.Z | sorttable -r | select_first book | column delete_time book | rename delete_time finish | compute 'finish = substr(finish,1,6)' | jointable -1 book -2 book - author_book.rdb | column finish author book | sorttable -r | link_authorBook
